...
Trong $\mathbb{R}^2$ cho điểm $M_0(x_0,y_0)$ và số thực $\epsilon>0$. Lân cận của $M_0$ bán kính $\epsilon$ là tập hợp

$N_{\epsilon}(M_0)=\begin{Bmatrix}M(x,y)\in \mathbb{R}^2: MM_0<\epsilon\end{Bmatrix}$

#### + Điểm trong - In Point
Cho tập $S\subset \mathbb{R}^2$ và điểm $M_0\in\mathbb{R}^2$. $M_0$ được gọi là điểm trong của $S$ khi và chỉ khi

$$\exists N_{\epsilon}(M_0)\subset S$$
